        ...
        if (Utils.deviceHasCameraFlash(mContext)) {
            try {
                ...
                if (!mUseCameraInterface) {
                    ...
                    // them kernel developerses should look at the commit message here (git blame)
                    Log.w(LOG_TAG, "Lightbulb is falling back to the interface. " +
                            "If you are on a custom kernel, contact your kernel developer " +
                            "for sysfs support.", e);
                    ...
                    Log.w(LOG_TAG, "Lightbulb was not able to switch modes " +
                            "(no fallback is available).", e);
                }
            }
        } else {
            mFlashMode = mode;
